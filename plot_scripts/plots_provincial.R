...
data_prov %>%
  ...
  labs(title = "Totaal positief-geteste Coronavirus besmettingen per provincie") +
...
for (i in seq(0, 6)){
  ...
  p = data_subset_map %>%
    ggplot() +
    geom_sf(aes(fill=AantalCumulatief, color=AantalCumulatief, geometry = geometry)) + coord_sf( expand = FALSE) +
    theme_minimal() +
    theme(panel.grid.major = element_line(colour = "transparent"),
          axis.text.x=element_blank(),
          axis.text.y=element_blank(),
...
pgrid = plot_grid(plotlist=p_list,
                  ncol=4) +
  ggsave("plots/map_province_cum.png", width = 6, height=4)
